...
def bfs(graph, root, max_depth):
    ...
    while d:
        par_node = d.popleft()
        for n in graph.neighbors(par_node):
            if( n not in visited):
                if(max_depth > node2distances[par_node]):
                    visited.append(n)
                    d.append(n)
                    node2num_paths[n] = 1
                    node2distances[n] = node2distances[par_node] + 1
                    node2parents[n].append(par_node)
            else:
                if(node2distances[n] == node2distances[par_node] + 1):
    ...
    #print(node2distances.items(), node2parents.items(), node2num_paths.items())
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    #pass
    distances={}
    keys = [k for k,v in node2distances.items()]
    for k in keys:
        if k == root:
            distances[k]= 0.0
        else:
    ...
    sortedNodes = sorted(node2distances.items(), key=lambda x:(x[1]), reverse = True)
    for k,v in sortedNodes:
        if(node2num_paths[k] == 1):
            for parent in node2parents[k]:
                  distances[parent] = distances[parent] + distances[k]
        elif(node2num_paths[k] > 1):
            for ele in node2parents[k]:
                distances[ele] = distances[ele] + (1.0/node2num_paths[k])
                distances[k] = 1.0/node2num_paths[k] 
            
                    
    res={}
    
    nodekeys = [k for k,v in distances.items()]

    for k in nodekeys: 
        for parent in node2parents[k]:
            temp = (k,parent)
            tup = sorted(temp)
            res[(tup[0],tup[1])]=distances[k]
    
    #print(sorted(ans.items()))
    sorted_ans = sorted(res.items(), reverse = True)
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in nodes:
        ...
        #print("length of each result",len(sorted(result)))
        for k,v in result.items():
            finalResult[k] = finalResult[k] + v
    
    keys = [k for k,v in finalResult.items()]
    for k in keys:
        finalResult[k] = finalResult[k]/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    count=0;
    G.remove_edge(*(approx_bet[count][0]))
    subgraphs =[x for x in nx.connected_component_subgraphs(G)]
    n = len(subgraphs)
...
def get_subgraph(graph, min_degree):
    ...
    dgraph = graph.degree()
    for d in dgraph:
        if( min_degree <= dgraph[d] ):
            sgraph.add_node(d)
    return graph.subgraph(sgraph)
...
def norm_cut(S, T, graph):
    ...
    ###TODO
    #pass
    ncut= ((cut(S, T, graph)/float(volume(S, graph))) + (cut(S, T, graph)/float(volume(T, graph))))
    return ncut
...
def score_max_depths(graph, max_depths):
    ...
    for max_depth in max_depths:
        #print("partition girvan res",partition_girvan_newman(graph, max_depth))
        cluster = partition_girvan_newman(graph, max_depth)
        S = list(cluster[0].nodes())
        T = list(cluster[1].nodes())
        ncut = norm_cut(S, T, graph)
        list_tuples.append((max_depth, ncut))
        #print(list_tuples)
    return list_tuples
...
def make_training_graph(graph, test_node, n):
    ...
    G_train = graph.copy()
    count = 1
    for e in sorted(G_train.edges(test_node)):
        if(count <= n):
            count = count + 1
...
def jaccard(graph, node, k):
    ...
    sortedScores = sorted(scores, key=lambda x: (-x[1], x[0]))
...
def path_score(graph, root, k, beta):
    ...
    sortedScores= (sorted(scores, key=lambda t: (-t[1],t[0])))
    list_scores = []
    count = 1
...
def main():
    ...
    graph = read_graph()
    #graph = example_graph()
    """node2distances, node2num_paths, node2parents = bfs(graph, 'Walgreens', 2)
    result = bottom_up('Walgreens', node2distances, node2num_paths, node2parents)
    print(len(result))
    print(len(approximate_betweenness(graph,2)))
    
    partition_girvan_newman(graph, 2)
    """
    #sorted(result.items())
